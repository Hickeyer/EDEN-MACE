...
DisRankParam.initColumn = function () {
    return [
        ...
        {title: '平台级别', field: 'disUserRankDetail', visible: true, align: 'center', valign: 'middle'},
...
DisRankParam.openAddDisRankParam = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/DisRankParamAgent/DisRankParam_add'
    });
...
DisRankParam.openDisRankParamDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/DisRankParamAgent/DisRankParam_update/' + DisRankParam.seItem.id
        });
        this.layerIndex = index;
    }
};
...
DisRankParam.delete = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/DisRankParamAgent/delete", function (data) {
            Feng.success("删除成功!");
...
$(function () {
    ...
    var table = new BSTable(DisRankParam.id, "/DisRankParamAgent/list", defaultColunms);
