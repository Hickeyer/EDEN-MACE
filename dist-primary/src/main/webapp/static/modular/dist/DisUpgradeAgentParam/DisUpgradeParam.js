...
DisUpgradeParam.openAddDisUpgradeParam = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/DisUpgradeParam/agent/DisUpgradeParam_add'
    });
...
DisUpgradeParam.delete = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/DisUpgradeParam/agent/delete", function (data) {
            Feng.success("删除成功!");
...
$(function () {
    ...
    var table = new BSTable(DisUpgradeParam.id, "/DisUpgradeParam/agent/list", defaultColunms);
