...
Actions.prototype.init = function()
{
	...
	this.addAction('save', function() {
		// alert(pageType);
		// 此处为保存事件触发按钮
       // alert(mxUtils.getPrettyXml(ui.editor.getGraphXml()));
        console.log(mxUtils.getPrettyXml(ui.editor.getGraphXml()));
        var reuslt = mxUtils.getPrettyXml(ui.editor.getGraphXml());
        $.ajax({
            type: 'POST',
            url: "/test",
            data: {"xml":reuslt,"pageType":pageType},
	...
	this.addAction('editData...', function()
	{
		...
		console.log(cell);
		/*if(!!cell.value.attributes[0]){
            console.log(cell.value.attributes[0].nodeValue);
		}
*/
